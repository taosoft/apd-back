...
exports.getDesperfecto = async (req, res, next) => {
    try {
        const desperfectos = await DesperfectoService.getDesperfecto(req.params.id);

        return res.status(200).json({
            status: 200,
            data: desperfectos,
            message: "Desperfeto encontrados exitosamente",
        });
    } catch (e) {
        return res.status(400).json({ status: 400, message: e.message });
    }
